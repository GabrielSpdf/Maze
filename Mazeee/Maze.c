...
int main()
{
	...
	jogador = player_start(mapa, &jogador); /*Encontrar posicao inicial do jogador dentro do mapa proposto*/
	saida = escape_pos(mapa, &saida); /*Encontrar posicao da saida dentro do mapa proposto*/
	...
	while(check_end(&jogador, &saida, arvore) != 0) /*Enquanto o jogador não chegar na posição final*/
	{
		...
		print_maze(mapa, &jogador, &saida); /*Imprimir labirinto*/
